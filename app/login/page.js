...
const firebaseConfig = {
  apiKey: "AIzaSyCoKlyT3Z9HJroxfAqnH6rtanVZX07Qd2w",
  authDomain: "workoutgen-b0cbd.firebaseapp.com",
  projectId: "workoutgen-b0cbd",
  storageBucket: "workoutgen-b0cbd.firebasestorage.app",
  messagingSenderId: "160888616958",
  appId: "1:160888616958:web:bed7a2a327da097ad5b2af",
  measurementId: "G-SBC7H0LDFM"
};
...
const page = () => {
    ...
    const clickhandle = async () => {
        try {
            // Set auth persistence to SESSION before signing in
            ...
            if (error.code === 'auth/network-request-failed') {
                ...
                // Ignore this error as it's common when user clicks multiple times
                return;
            } else {
    ...
    const signouthandle = async () => {
        try {
            ...
            setcurrentUser(null);
            // Optionally force reload the page
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                setcurrentUser(null);  // Update state to reflect that no user is logged in
            }
        });
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-to-br from-[#03624c]/30 via-black to-[#00df82]/30 animate-gradient-fast"></div>
        {/* Brighter gradient orbs with enhanced glow */}
        <div className="absolute top-[10%] left-[15%] w-[6rem] h-[6rem] bg-[#00df82]/40 rounded-full blur-[40px] animate-blob1-fast"></div>
        <div className="absolute bottom-[20%] right-[10%] w-[6rem] h-[6rem] bg-[#03624c]/40 rounded-full blur-[40px] animate-blob2-fast"></div>
        <div className="absolute top-[40%] right-[25%] w-[6rem] h-[6rem] bg-[#00df82]/30 rounded-full blur-[40px] animate-blob3-fast filter contrast-125"></div>
        <div className="absolute bottom-[30%] left-[25%] w-[6rem] h-[6rem] bg-[#03624c]/30 rounded-full blur-[40px] animate-blob4-fast filter contrast-125"></div>
        <div className="absolute top-[60%] left-[45%] w-[6rem] h-[6rem] bg-[#00df82]/35 rounded-full blur-[40px] animate-blob5-fast"></div>
        <div className="absolute top-[20%] right-[40%] w-[6rem] h-[6rem] bg-[#03624c]/35 rounded-full blur-[40px] animate-blob6-fast"></div>
        <div className="absolute bottom-[40%] right-[45%] w-[6rem] h-[6rem] bg-[#00df82]/30 rounded-full blur-[40px] animate-blob7-fast filter contrast-125"></div>
      </div>

      {/* Content Container with enhanced lighting */}
      <div className="relative z-10 w-full max-w-md px-6 py-12">
        <div className="bg-black/50 backdrop-blur-xl rounded-2xl border border-[#00df82]/20 p-8 shadow-2xl shadow-[#00df82]/10">
          <h2 className="text-4xl font-bold mb-8 text-center bg-gradient-to-r from-[#00df82] to-[#03624c] bg-clip-text text-transparent">
            Welcome Back
          </h2>

          {currentUser ? (
            <div className="space-y-4 text-center">
              <h3 className="text-xl text-gray-200">Login info</h3>
              <p className="text-[#00df82]">{currentUser.displayName}</p>
              <p className="font-bold text-white">{currentUser.email}</p>
              <button 
                onClick={signouthandle}
                className="w-full px-6 py-3 bg-gradient-to-r from-[#03624c] to-[#00df82] hover:from-[#00df82] hover:to-[#03624c] text-white rounded-xl transition-all duration-300 font-medium"
              >
                Sign Out
              </button>
            </div>
            ...
            <div className="space-y-4">
              <p className="text-gray-400 text-center mb-6">Choose your sign-in method</p>
              <div className="grid gap-3">
                {/* Existing buttons with enhanced styling */}
                <button
                  onClick={clickhandle}
                  className="flex items-center w-full bg-white/10 backdrop-blur-sm text-white border border-white/10 rounded-xl shadow-md px-6 py-3 text-sm font-medium hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-[#00df82]/50 transition-all duration-300"
                >
                ...
                {/* Apply similar styling to other social buttons */}
                <button
                  className="flex items-center w-full bg-white/10 backdrop-blur-sm text-white border border-white/10 rounded-xl shadow-md px-6 py-3 text-sm font-medium hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-[#00df82]/50 transition-all duration-300">
                  <svg className="h-5 w-5 mr-3" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
